/*
# proc: binary_subimage_or_8 - uses a logical OR operator to copy a binary
# proc:                        subimage that is a multiple of 8 bits wide.
# proc: binary_subimage_or_gt - special condition when using a logical OR
# proc:                         operator to copy a binary subimage.
# proc: binary_subimage_or_lt - special condition when using a logical OR
# proc:                         operator to copy a binary subimage.
# proc: binary_subimage_or_eq - special condition when using a logical OR
# proc:                         operator to copy a binary subimage.
# proc: binary_subimage_or - uses a logical OR operator to copy a binary
# proc:                      subimage to another image at a specified
# proc:                      location.
*/
...
void binary_subimage_or_8 (
	...
	int srcx,srcy,dstx,dsty, srcw,srch,dstw,dsth, cpw,cph;
{
...
if (src == (u_char *) NULL)
	fatalerr("binary_subimage_or_8","Null source image pointer",(char *)NULL);

if (dst == (u_char *) NULL)
	fatalerr("binary_subimage_or_8","Null destination image pointer",(char *)NULL);

if ((srcw < 0) || (srch < 0))
	fatalerr("binary_subimage_or_8","Negative source image dimension(s)",(char *)NULL);

if ((dstw < 0) || (dsth < 0))
	fatalerr("binary_subimage_or_8","Negative destination image dimension(s)",(char *)NULL);

if ((cpw < 0) || (cph < 0))
	fatalerr("binary_subimage_or_8","Negative subimage dimension(s)",(char *)NULL);

if (srcw % BITSPERBYTE)
	fatalerr("binary_subimage_or_8",
...
if (src == (u_char *) NULL)
	fatalerr("binary_subimage_or_gt","Null source image pointer",(char *)NULL);

if (dst == (u_char *) NULL)
	fatalerr("binary_subimage_or_gt","Null destination image pointer",(char *)NULL);

if ((srcw < 0) || (srch < 0))
	fatalerr("binary_subimage_or_gt","Negative source image dimension(s)",(char *)NULL);

if ((dstw < 0) || (dsth < 0))
	fatalerr("binary_subimage_or_gt","Negative destination image dimension(s)",(char *)NULL);

if ((cpw < 0) || (cph < 0))
	fatalerr("binary_subimage_or_gt","Negative subimage dimension(s)",(char *)NULL);

if (srcw % BITSPERBYTE)
	fatalerr("binary_subimage_or_gt",
		"Source image width must be a multiple of 8",(char *)NULL);
...
j_inv = BITSPERBYTE - j;
		...
		diff = j_inv - b;
		if (diff <= 0)
		fatalerr("binary_subimage_or_gt","bad diff",(char *)NULL);
...
if (!i) {					/* Src is byte-aligned. */
	...
	while (cph--) {
		...
		binary_fill_partial(BINARY_OR,src,i,dst,j,b);
		src += dx;
		dst += dy;
	}
	return;
...
if (src == (u_char *) NULL)
	fatalerr("binary_subimage_or_lt","Null source image pointer",(char *)NULL);

if (dst == (u_char *) NULL)
	fatalerr("binary_subimage_or_lt","Null destination image pointer",(char *)NULL);

if ((srcw < 0) || (srch < 0))
	fatalerr("binary_subimage_or_lt","Negative source image dimension(s)",(char *)NULL);

if ((dstw < 0) || (dsth < 0))
	fatalerr("binary_subimage_or_lt","Negative destination image dimension(s)",(char *)NULL);

if ((cpw < 0) || (cph < 0))
	fatalerr("binary_subimage_or_lt","Negative subimage dimension(s)",(char *)NULL);

if (srcw % BITSPERBYTE)
	fatalerr("binary_subimage_or_lt",
		"Source image width must be a multiple of 8",(char *)NULL);
...
j_inv = BITSPERBYTE - j;
		...
		diff = j_inv - b;
		if (diff >= 0)
		fatalerr("binary_subimage_or_lt","bad diff",(char *)NULL);
...
if (src == (u_char *) NULL)
	fatalerr("binary_subimage_or_eq","Null source image pointer",(char *)NULL);

if (dst == (u_char *) NULL)
	fatalerr("binary_subimage_or_eq","Null destination image pointer",(char *)NULL);

if ((srcw < 0) || (srch < 0))
	fatalerr("binary_subimage_or_eq","Negative source image dimension(s)",(char *)NULL);

if ((dstw < 0) || (dsth < 0))
	fatalerr("binary_subimage_or_eq","Negative destination image dimension(s)",(char *)NULL);

if ((cpw < 0) || (cph < 0))
	fatalerr("binary_subimage_or_eq","Negative subimage dimension(s)",(char *)NULL);

if (srcw % BITSPERBYTE)
	fatalerr("binary_subimage_or_eq",
		"Source image width must be a multiple of 8",(char *)NULL);
...
j_inv = BITSPERBYTE - j;
		...
		diff = j_inv - b;
		if (diff)
		fatalerr("binary_subimage_or_eq","bad diff",(char *)NULL);
...
b = cpw % BITSPERBYTE;
if (!b) {
	binary_subimage_or_8(src,srcw,srch,
				dst,dstw,dsth,
...
if (diff > 0) {
	binary_subimage_or_gt(src,srcw,srch,
				dst,dstw,dsth,
				srcx,srcy,cpw,cph,dstx,dsty);
	return;
}
...
if (diff < 0) {
	binary_subimage_or_lt(src,srcw,srch,
				dst,dstw,dsth,
				srcx,srcy,cpw,cph,dstx,dsty);
	return;
}

	binary_subimage_or_eq(src,srcw,srch,
				dst,dstw,dsth,
				srcx,srcy,cpw,cph,dstx,dsty);
}
